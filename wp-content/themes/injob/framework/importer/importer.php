...
function inwave_importer() {
    ...
    switch ($step) {
        ...
        case 'media':
            @set_time_limit( 0 );
            $media_sources = array('media_source_1.zip', 'media_source_2.zip');
            ...
            $percent = 10 + round(($importer['media']/count($media_sources))*35, 0);

            break;
        case 'posts':
            if($importer['base']->importing()){
                $percent = 50 + round(($importer['base']->total_posts - count($importer['base']->posts))/ $importer['base']->total_posts * 25, 0);
            }else{
                array_shift($importer['steps']);
                $importer['step_done']++;
                $percent = 70;
