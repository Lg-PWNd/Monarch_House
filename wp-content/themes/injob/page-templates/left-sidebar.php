 ...
 * This is the template with left sidebar
 */
get_header();
		...
		<div class="row">
			...
			if ( ! $public_page || ( $public_page && is_user_logged_in() ) ) { ?>
				...
				<?php if ( $sidebar_position && $sidebar_position != 'none' ) { ?>
					<div class="<?php echo esc_attr( inwave_get_classes( 'sidebar', $sidebar_position, 'large' ) ) ?> <?php echo( $sidebar_sticky ? 'iwj-sidebar-sticky' : '' ); ?> iwj-sidebar-1">
						<?php get_sidebar(); ?>
					</div>
				<?php }
			} else {
				...
				<div class="iwj-candidate-non-permission">
					<?php echo sprintf( __( 'You must be logged in to view this page. <a href="%s">Login here</a>', 'iwjob' ), add_query_arg( 'redirect_to', $current_url, $login_page_id ) ); ?>
				</div>
				<?php
			}
			?>
