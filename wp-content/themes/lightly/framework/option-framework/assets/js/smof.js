...
jQuery(document).ready(function($){
	...
	//Add new option
	$(".addoption_add_button").live('click', function(e){
		e.preventDefault();
		var self = $(this);
		var slidesContainer = $(this).prev();
		var sliderId = slidesContainer.attr('id');
		...
		var preValue = self.attr('href')+' '+ newNum;
		var newSlide = '<li class="temphide"><div class="slide_header"><strong>'+ preValue + '</strong><input type="hidden" class="slide of-input order" name="' + sliderId + '[' + newNum + '][order]" id="' + sliderId + '_slide_order-' + newNum + '" value="' + newNum + '"><a class="slide_edit_button" href="#">Edit</a></div><div class="slide_body" style="display: none; "><input class="slide of-input of-slider-title" name="' + sliderId + '[' + newNum + '][option]" id="' + sliderId + '_' + newNum + '_slide_title" value="'+preValue+'"><a class="slide_delete_button" href="#">Delete</a><div class="clear"></div></div></li>';
	...
	//Add new link social
	$(".add_iconlink_add_button").live('click', function(){
		var slidesContainer = $(this).prev();
		var sliderId = slidesContainer.attr('id');
		...
		var newSlide = '<li class="temphide"><div class="slide_header"><strong>Social ' + newNum + '</strong><input type="hidden" class="slide of-input order" name="' + sliderId + '[' + newNum + '][order]" id="' + sliderId + '_slide_order-' + newNum + '" value="' + newNum + '"><a class="slide_edit_button" href="#">Edit</a></div><div class="slide_body" style="display: none; "><label>Title</label><input class="slide of-input of-slider-title" name="' + sliderId + '[' + newNum + '][title]" id="' + sliderId + '_' + newNum + '_slide_title" value=""><label>Icon</label><input class="slide of-input of-slider-icon" name="' + sliderId + '[' + newNum + '][icon]" id="' + sliderId + '_' + newNum + '_slide_icon" value=""><label>Link</label><input class="slide of-input" name="' + sliderId + '[' + newNum + '][link]" id="' + sliderId + '_' + newNum + '_slide_link" value=""><a class="slide_delete_button" href="#">Delete</a><div class="clear"></div></div></li>';
	...
	$('#of_backup_button').live('click', function(){
		...
		if (answer){
			...
			var data = {
				action: 'inwave_of_ajax_post_action',
				type: 'backup_options',
				security: nonce
			};
	...
	$('.section-accordion').toggle(function(e) {
		e.preventDefault();
		$(this).parents('.accordion').find('.section-accordion .dashicons').removeClass('dashicons-plus').addClass('dashicons-minus');
		$(this).parents('.accordion').find('.section').slideDown();
	}, function(e) {
		e.preventDefault();
		$(this).parents('.accordion').find('.section-accordion .dashicons').removeClass('dashicons-minus').addClass('dashicons-plus');
		$(this).parents('.accordion').find('.section').not('.section-accordion').slideUp();
	});
    var importing = false;

    $('#section-demo_data .button-primary').click(function(e){
        if(importing){
            alert('Please wait..');
            e.preventDefault();
            return false;
        ...
        if(confirm('Are you sure you want to import?')){
            $('#section-demo_data .button-primary').html('Preparing ...');
            $('.iw-loading-progress').addClass('importing').slideDown();
            importing = true;
            var data = {
                action: 'inwave_importer',
                iw_stage: 'init',
            };

            sendImportRequest(data);
        }

        e.preventDefault();
        return false;
    });
    function sendImportRequest(data){
        $.post(ajaxurl, data, function(response) {
            //check nonce
            try {
                ...
                $('.iw-loading-info').html('Progress: '+ response.percent);
                $('.iw-import-output').html(response.msg + ' ...');
                $('.iw-loading-state').width(response.percent);

            } catch(err) {
                $('.iw-import-output').html('<h3>Output Error:</h3><textarea style="width:100%;height:300px">'+response+'</textarea>');
                returnStartState();
            }
    ...
    function returnStartState(){
        importing = false;
        $('#section-demo_data .button-primary').html('Import Demo Content');
    }
    $('#of_container input[type=checkbox]').iCheck();
}); //end doc ready