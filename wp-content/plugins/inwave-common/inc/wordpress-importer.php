 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
	class WP_Import_Extend extends WP_Importer {
		...
		var $attachment_mapping = array();

		var $fetch_attachments = false;
		var $url_remap = array();
		var $featured_images = array();

		/* added in extend version */
		var $allow_post_types = array();
		var $allow_term_types = array();
		var $exclude_attachment_types = array();
		var $debug = false;
		var $message = '';
		var $error_msg = '';
		var $step_total = 1;
		...
		//custom by hoa
		var $imported_post_type;
		var $imported_attachment_post;
		...
		function import_start( $file ) {
			...
			/** debug import types */
			if($this->debug) {
				$tmp_posts = $import_data['posts'];
				$import_data['posts'] = array();
				foreach ($tmp_posts as $item) {
					if (isset($import_data['posts'][$item['post_type']])) {
						$import_data['posts'][$item['post_type']][] = $item;
					} else {
						$import_data['posts'][$item['post_type']] = array();
						$import_data['posts'][$item['post_type']][] = $item;
					}
				}
				echo "POST TYPE REQUEST:\n";
				foreach($this->allow_post_types as $key => $value){
					echo '-'. $value."-\n";
				}
				echo "POST TYPE XML FILE:\n";
			...
			foreach($import_data['posts'] as $post){
				if(in_array($post['post_type'],$this->allow_post_types)){
					if($post['post_type'] == 'attachment'){
						$media_type = explode('-',$post['post_title']);
						$media_type = trim($media_type[0]);
						if(in_array($media_type,$this->exclude_attachment_types)){
							continue;
						}
			...
			$import_data['posts'] = $checkedPosts;

			// remove invalid terms
			$checkedTerms = array();
			foreach($import_data['terms'] as $term){
				if(in_array($term['term_taxonomy'],$this->allow_term_types)){
					$checkedTerms[] = $term;
				}
			}
			$import_data['terms'] = $checkedTerms;
			...
			if(in_array('post',$this->allow_post_types)) {
		...
		function import_options() {
			...
			<form action="<?php echo admin_url( 'admin.php?import=wordpress&amp;step=2' ); ?>" method="post">
				...
				<?php if ( ! empty( $this->authors ) ) : ?>
					...
					<p><?php echo wp_kses_post(__( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', 'wordpress-importer' )); ?></p>
		...
		function process_posts() {
            ...
            $users_mapping = $wp_filesystem->get_contents(get_template_directory() . '/framework/importer/data/users_mapping.txt');
            if($users_mapping){
                $this->author_mapping = json_decode($users_mapping, true);
            }
            ...
            foreach ( $this->posts as $post ) {
                ...
                $this->imported_post_type = $post['post_type'];

				if($post['post_type'] == 'attachment'){
					$attachment_mapping[$original_post_ID] = $post_id;
                    $this->imported_attachment_post++;
                    if($this->imported_attachment_post % 10 == 0) {
                        break;
                    }
				}

				if($this->imported_post_type && $post['post_type'] != $this->imported_post_type){
				    break;
                }

				/*if($post['post_type'] == 'attachment' && ! $this->message) {
					$this->message = 'Import media: '. $post['post_title'];
					break;
				}*/
            }

			$this->posts= array_slice($this->posts,$processed);
		}
		...
		function process_menu_item( $item ) {
			...
			foreach ( $item['postmeta'] as $meta ){
				$variable_name = $meta['key'];
				$$variable_name = $meta['value'];
			}
			...
			if ( $id && ! is_wp_error( $id ) ){
				...
				if (isset($_bg_submenu)) {
					update_post_meta( $id, '_bg_submenu', $_bg_submenu );
				}
				if (isset($_number_column)) {
					update_post_meta( $id, '_number_column', $_number_column );
				}
			}
		}
		...
		function process_attachment( $post, $url ) {
			...
			$upload = $this->_fetch_remote_file( $url, $post );
			if ( is_wp_error( $upload ) ){
				$this->error_msg =  $upload->get_error_message();
				return;
			}
			...
			if ( $info = wp_check_filetype( $upload['file'] ) )
				...
				$this->error_msg = 'attachment_processing_error:' . $url .' '. esc_html__('Invalid file type', 'wordpress-importer');
				return;
			}
		 ...
		 */
		function _fetch_remote_file( $url, $post ) {
            ...
            if(!$wp_filetype || !$wp_filetype['type']){
                return new WP_Error('1', 'can not check file type image');
            }

            return array( 'file' => $new_file, 'url' => $url, 'type' => $wp_filetype['type'], 'error' => false );
		}
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( $upload['error'] ){
				$this->error_msg = 'upload_dir_error:' . $url .' ' . $upload['error'];
				return;
			}
			...
			if ( ! $headers ) {
				@unlink( $upload['file'] );
				$this->error_msg =  'import_file_error: '. $url .' '.__('Remote server did not respond', 'wordpress-importer');
				return;
			}
			...
			if ( $headers['response'] != '200' ) {
				@unlink( $upload['file'] );
				$this->error_msg =  'import_file_error: '. $url .' '.sprintf( __('Remote server returned error response %1$d %2$s', 'wordpress-importer'), esc_html($headers['response']), get_status_header_desc($headers['response']) ) ;
				return ;
			}
			...
			if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
				@unlink( $upload['file'] );
				$this->error_msg = 'import_file_error: ' . $url .' '. __('Remote file is incorrect size', 'wordpress-importer') ;
				return;
			}

			if ( 0 == $filesize ) {
				@unlink( $upload['file'] );
				$this->error_msg = 'import_file_error: ' . $url .' '. __('Zero size file downloaded', 'wordpress-importer') ;
				return;
			}
			...
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				@unlink( $upload['file'] );
				$this->error_msg = 'import_file_error: ' . $url .' '.sprintf(__('Remote file is too large, limit is %s', 'wordpress-importer'), size_format($max_size) )  ;
				return;
			}
