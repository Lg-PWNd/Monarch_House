        ...
        <table class="table">
            <thead>
                <tr class="package-heading">
                    ...
                    <th width="15%" class="text-center"><?php echo __('Jobs Remaining', 'iwjob');?></th>
            ...
            <?php if($user_package_query->have_posts()) {
                ...
                while ($user_package_query->have_posts()){
                    ...
                    if($user_package->has_status('pending-payment')){
                        if($package){
                            $remain_job = $package->get_number_job();
                            $remain_renew_job = $package->get_number_renew_job();
                            $remain_featured_job = $package->get_number_featured_job();
                        }else{
                            $remain_job = $remain_renew_job = $remain_featured_job = __('N/A', 'iwjob');
                        }
                    }else{
                        $remain_job = $user_package->get_remain_job();
                        $remain_renew_job = $user_package->get_remain_renew_job();
                        $remain_featured_job = $user_package->get_remain_featured_job();
                    }
                ...
                <tr class="iwj-empty">
                    <td colspan="7"><?php echo __('You have not any packages yet.', 'iwjob'); ?></td>
                </tr>
            <?php } ?>
            </tbody>
        </table>
