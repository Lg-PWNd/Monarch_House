...
<?php do_action('iwj_before_widget_candidate_filter', $args, $instance, $parent); ?>
...
<?php if (isset($list_taxonomies) && is_array($list_taxonomies) && $list_taxonomies) : ?>
    ...
    <div id="iwj-clear-filter-btn" class="facet open-filters" style="display: none;">
        <button class="btn btn-primary" id="clear-filter-candidate">
    ...
    <?php foreach($list_taxonomies as $item_taxonomy) : ?>
        ...
        <?php if (!empty($list_taxonomies_data[$item_taxonomy])) :
            ...
            $is_tree = in_array($item_taxonomy, iwj_taxonomy_widget_tree_show('candidate'));
            ?>
            <aside class="widget sidebar-jobs-item">
                <h3 class="widget-title"><span><?php echo $taxonomy_label; ?></span></h3>
                <div class="sidebar-job-1">
                    <form name="<?php echo $item_taxonomy; ?>" class="candidate-form-filter form-<?php echo $item_taxonomy; ?>">
                        ...
                        <ul class="iwjob-list-<?php echo $item_taxonomy.($is_tree ? ' iwj-tax-tree' : ''); ?>">
                            <?php
                            if($is_tree){
                                iwj_walk_tax_tree( $list_taxonomies_data[$item_taxonomy], 0, $filters_data['taxonomy_ids'], $iwj_limit, 'candidates', $item_taxonomy );
                            }else{
                                $i = 1;
                                    ...
                                    <li <?php echo $style; ?> class="theme-color-hover iwj-input-checkbox <?php echo $item_taxonomy; ?>" data-order="<?php echo $term->total_post; ?>">
                                        <div class="filter-name-item">
                                            <input type="checkbox" <?php echo iwj_filter_tax_checked($term->term_id, $filters_data['taxonomy_ids']); ?> name="<?php echo $item_taxonomy; ?>[]"
                                                   id="iwjob-filter-candidates-cbx-<?php echo $term->term_id; ?>" class="iwjob-filter-candidates-cbx"
                                                   value="<?php echo $term->term_id; ?>" data-title="<?php echo $term->name; ?>">
                                            <label for="iwjob-filter-candidates-cbx-<?php echo $term->term_id; ?>"><?php echo $term->name; ?></label>
                                        </div>
            ...
            <?php do_action('iwj_widget_candidate_filter_after_'.$item_taxonomy, $args, $instance, $parent); ?>

        <?php endif; ?>

    <?php endforeach; ?>

    <form name="iwjob-other-filters" class="candidate-form-filter form-other-filters">
        <?php
...
<?php do_action('iwj_after_widget_candidate_filter', $args, $instance, $parent); ?>

<input type="hidden" value="candidate" name="wgit_job_filter" id="wgit-job-filter" />
