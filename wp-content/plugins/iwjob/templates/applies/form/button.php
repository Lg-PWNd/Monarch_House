...
<?php if(!$user && !iwj_option('allow_guest_apply_job')){ ?>
    <a href="#" class="apply-job" data-toggle="modal" data-target="#iwj-login-popup" data-fallback="show_apply_form"><i class="ion-android-checkbox-outline"></i><?php echo __('Apply for job', 'iwjob'); ?></a>
<?php }else {
    ...
    <?php } else { ?>
        <a href="#" class="apply-job" data-toggle="modal" data-target="#iwj-modal-apply-<?php echo get_the_ID(); ?>">
            <i class="ion-android-checkbox-outline"></i><?php echo __('Apply for job', 'iwjob'); ?>
        </a>
        <?php
        wp_enqueue_style('iwj-apply-form');
        wp_enqueue_script('iwj-apply-form');
        iwj_get_template_part('applies/form/popup', array('job' => $job, 'self'=>$self));
    }
}
?>
