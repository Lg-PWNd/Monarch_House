...
function iwj_get_available_languages() {
    ...
    if($languages){
        ...
        foreach ( $languages as $language ) {
            ...
            $chosen_languages[$language] = $language;
...
function iwj_countries_list() {
    ...
    $Countries = array(
        ...
        'BL' => esc_html__('Saint Barth̩lemy', 'iwjob'),
...
function iwj_get_term_hierarchy($taxonomy, $parent = 0, $level = 0, $args = array()){
    ...
    if($childterms){
        ...
        foreach ($childterms as $childterm){
            ...
            $newterms = iwj_get_term_hierarchy($taxonomy, $childterm->term_id, $level);
...
function iwj_get_cats($cat_ids = array(), $exclude_ids = array(), $hide_empty = false, $limit = '', $order_by = 'count', $oder = 'DESC'){
    ...
    if(!isset($cats[$key])){
        ...
        $join_sql .= " LEFT JOIN {$wpdb->term_relationships} AS tr ON tr.object_id = p.ID";

        $join_sql .= " LEFT JOIN {$wpdb->term_taxonomy} AS tt ON tt.term_taxonomy_id = tr.term_taxonomy_id";

        $join_sql .= " LEFT JOIN {$wpdb->terms} AS t ON t.term_id = tt.term_id";



        $where_sql = "p.post_type = 'iwj_job' AND p.post_status = 'publish' AND tt.taxonomy = 'iwj_cat'";

        if(!iwj_option('show_expired_job')){

            $join_sql .= " LEFT JOIN {$wpdb->postmeta} AS pm ON pm.post_id = p.ID";

            $where_sql .= " AND pm.meta_key = '".IWJ_PREFIX."expiry' AND (pm.meta_value = '' OR CAST(pm.meta_value AS UNSIGNED) > ".current_time('timestamp').")";
        ...
        if($cat_ids){
            ...
            $where_sql .= " AND t.term_id IN (".implode(',', $cat_ids).")";
        ...
        if($exclude_ids){
            ...
            $where_sql .= " AND t.term_id NOT IN (".implode(',', $exclude_ids).")";
        ...
        $sql = "SELECT t.*, COUNT(p.ID) AS total FROM {$wpdb->posts} AS p {$join_sql} WHERE {$where_sql} GROUP BY tt.term_id {$having} ORDER BY {$oder_by_sql} {$oder} $limit_sql";
    ...
    return apply_filters('iwj_get_cats', $categories);

}



function iwj_get_employers($employer_ids = array(), $exclude_ids = array(), $hide_empty = false, $limit = '', $order_by = 'count', $oder = 'DESC'){
    ...
    if(!isset($employers[$key])){
        ...
        $join_sql .= " LEFT JOIN {$wpdb->posts} AS p1 ON p.`post_author` = p1.`post_author`";



        $where_sql = "p.post_type = 'iwj_employer' AND p.post_status = 'publish' AND  p1.post_type = 'iwj_job' AND p1.post_status = 'publish'";

        if(!iwj_option('show_expired_job')){

            $join_sql .= " LEFT JOIN {$wpdb->postmeta} AS pm ON pm.post_id = p1.ID";
        ...
        if($employer_ids){
            ...
            $where_sql .= " AND p.ID IN (".implode(',', $employer_ids).")";
        ...
        $sql = "SELECT COUNT(p1.ID) AS total_jobs, p.*  FROM {$wpdb->posts} AS p {$join_sql} WHERE {$where_sql} GROUP BY p1.post_author {$having} ORDER BY {$oder_by_sql} {$oder} $limit_sql";
...
function iwj_get_avatar($id_or_email, $size = '', $default = '', $alt = '', $args = null){
    ...
    if(!$size){
        ...
        $class = array( 'avatar', 'avatar-' . (!$size ? 'full' : $size), 'photo' );
        ...
        if(is_numeric($avatars)) {
            ...
            $avatar = sprintf(
                ...
                "<img alt='%s' src='%s' class='%s' width='%s' height='%s' %s/>",
        ...
        return apply_filters('iwj_get_avatar', $avatar, $id_or_email, $size, $default, $alt, $args);

    }else{

        return get_avatar($id_or_email, $size, $default = '', $alt = '', $args = null);
...
function iwj_get_dashboard_menus($show_on_position = 1){
    ...
    $menus = array(
        ...
        'overview' => array(
            ...
            'title' => '<i class="fa fa-user"></i>'.__('Dashboard', 'iwjob')
    ...
    if($user && $user->is_employer()){
        ...
        if($show_on_position == 1 || $show_on_position == 2){
            ...
            $menus['new-job'] = array(
                ...
                'title' => '<i class="fa fa-edit"></i>'.__('Add New Job', 'iwjob')
        ...
        $menus['jobs'] = array(
            ...
            'title' => '<i class="fa fa-suitcase"></i>'.__('All Jobs', 'iwjob')
        ...
        $menus['applications'] = array(
            ...
            'title' => '<i class="fa fa-newspaper-o"></i>'.__('Applications', 'iwjob'),
        ...
        if(iwj_option('view_free_resum') || current_user_can('privilege_view_resum')){
            /*
                $menus['save-resumes'] = array(
                    ...
                    'title' => '<i class="fa fa-flag-checkered"></i>'.__('Saved Resumes', 'iwjob')
            ...
            */
            ...
            $menus['view-resumes'] = array(
                ...
                'title' => '<i class="fa fa-flag-checkered"></i>'.__('Viewed Resumes', 'iwjob')
        ...
        if($show_on_position == 1 || $show_on_position == 2){
            ...
            if(iwj_option('include_my_account_woocommerce') || iwj_woocommerce_checkout()){
                ...
                if($has_orders) {
                    ...
                    $menus['w-orders'] = array(
                        ...
                        'title' => '<i class="fa fa-cart-plus"></i>' . ($show_order ? __('Shop Orders', 'iwjob') : __('Orders', 'iwjob'))
        ...
        if(current_user_can('administrator')){
            $menus['browse-listings'] = array(
                'url' => 'https://goldmedicaljobs.com/all-listings/',
                'title' => '<i class="fa fa-briefcase"></i>' . __('Browse Listings', 'iwjob'),
                'submenu' => array(
                    'search-map' => array(
                        'url' => 'https://goldmedicaljobs.com/search-map/',
                        'title' => __('Search Map', 'iwjob')
                    ),
                    'candidate-listings' => array(
                        'url' => 'https://goldmedicaljobs.com/candidates/',
                        'title' => __('Candidate Listings', 'iwjob')
                    ),
                    'employer-listings' => array(
                        'url' => 'https://goldmedicaljobs.com/employers/',
                        'title' => __('Employer Listings', 'iwjob')
                    ),
                    'members' => array(
                        'url' => 'https://goldmedicaljobs.com/members/',
                        'title' => __('Members', 'iwjob')
                    ),
                    'candidate-suggestion' => array(
                        'url' => 'https://goldmedicaljobs.com/candidate-suggestion/',
                        'title' => __('Candidate Suggestion', 'iwjob')
                    ),
                )
            );
        }
        ...
        /*
            ...
            if (!$disable_review){
                ...
                $menus['reviews'] = array(
                    ...
                    'title' => '<i class="fa fa fa-star-o"></i>' . __('Reviews', 'iwjob')
    ...
    if($user && $user->is_candidate()) {
        ...
        $menus['submited-applications'] = array(
            ...
            'title' => '<i class="fa fa-newspaper-o"></i>' . __('Applied Jobs', 'iwjob')
        ...
        */
        ...
        $menus['form'] = array(
            ...
            'url' => '',
            'title' => '<i class="fa fa-briefcase"></i>' . __('Form', 'iwjob'),
            'submenu' => array(
                'resume' => array(
                    'url' => 'https://goldmedicaljobs.com/resume-first/',
                    'title' => __('Resume', 'iwjob')
                ),
                'requirements' => array(
                    'url' => 'https://goldmedicaljobs.com/requirements/',
                    'title' => __('Requirements', 'iwjob')
                ),
                'nurse-skill-checklists' => array(
                    'url' => 'https://goldmedicaljobs.com/nurse-skills-checklist/',
                    'title' => __('Nurse skill checklists', 'iwjob')
                ),
                'doctor-skill-checklists' => array(
                    'url' => add_query_arg(array('iwj_tab' => 'doctor-skill-checklists'), $dashboard_url),
                    'title' => __('Doctor skill checklists', 'iwjob')
                ),
                'allied-skill-checklists' => array(
                    'url' => 'https://goldmedicaljobs.com/allied-skills-checklists/',
                    'title' => __('Allied skill checklists', 'iwjob')
                ),
            )
        ...
        $menus['save-jobs'] = array(
            ...
            'title' => '<i class="fa fa-heart"></i>' . __('Saved Jobs', 'iwjob')
        ...
        */
        ...
        $menus['request-portfolio'] = array(

            'url' => add_query_arg(array('iwj_tab' => 'request-portfolio'), $dashboard_url),

            'title' => '<i class="fa fa-star"></i>' . __('Request Portfolio', 'iwjob')
    ...
    $menus['logout'] = array(
        ...
        'title' => '<i class="fa fa-sign-out"></i>' . __('Logout', 'iwjob')
