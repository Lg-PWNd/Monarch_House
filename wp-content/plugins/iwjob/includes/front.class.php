...
class IWJ_Front{
    ...
    static function blockusers_init() {
        if (is_user_logged_in() && is_blog_admin() && ! current_user_can( 'administrator' ) &&
            ! ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
            if(current_user_can( 'iwj_candidate' ) || current_user_can( 'iwj_employer' )){
    ...
    static function enqueue_scripts(){
        ...
        wp_localize_script('iwj', 'iwj', array(
            ...
            'map_styles' => iwj_option('map_styles'),
    ...
    static function tab_title() {
        ...
        if (isset($_GET['iwj_tab']) &&  $_GET['iwj_tab'] ) {
            $iwj_tab_titles = array(
                ...
                'save-jobs' => __('Saved Jobs', 'iwjob'),
                'jobs' => __('All Jobs', 'iwjob'),
                'applications' => __('Applications', 'iwjob'),
                'packages' => __('Packages', 'iwjob'),
                'orders' => __('Orders', 'iwjob'),
                'save-resumes' => __('Saved Resumes', 'iwjob'),
