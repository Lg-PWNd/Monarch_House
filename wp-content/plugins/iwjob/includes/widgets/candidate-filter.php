...
class IWJ_Widget_Candidate_Filter extends WP_Widget {

    public function __construct() {
        $widget_ops = array(
            'description' => esc_html__( 'Filter Candidates Widget.', 'iwjob'),
            'customize_selective_refresh' => true,
        );
        parent::__construct( 'iwj_candidate_filter', esc_html__( '[IWJ] Filter Candidates', 'iwjob' ), $widget_ops );
    }

    public function widget( $args, $instance ) {
        //get taxonomies support
        $list_taxonomies = array();
        $job_taxonomies = iwj_get_candidate_taxonomies();
        ...
        iwj_get_template_part('widgets/candidate-filter', $data);
    }

    public function update( $new_instance, $old_instance ) {
        $instance = array();
        $instance['title'] = sanitize_text_field( $new_instance['title'] );
        $job_taxs = iwj_get_candidate_taxonomies();
        if($job_taxs){
        ...
        $instance = apply_filters('iwj_widget_candidate_filter_update', $instance, $new_instance, $old_instance, $this);

        return $instance;
    }

    public function form( $instance ) {
        $instance = wp_parse_args( (array) $instance, array( 'title' => __('Candidate Filter', 'iwjob') ) );
        ...
        <?php
        do_action('iwj_before_widget_from_candidate_filter', $this, $instance);
        ...
        do_action('iwj_after_widget_from_candidate_filter', $this, $instance);
    }
}

function iwj_widget_candidate_filter() {
    register_widget('IWJ_Widget_Candidate_Filter');
}
add_action('widgets_init', 'iwj_widget_candidate_filter');