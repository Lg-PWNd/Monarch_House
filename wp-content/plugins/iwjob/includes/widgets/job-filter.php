...
class IWJ_Widget_Job_Filter extends WP_Widget {
    ...
    public function __construct() {
        $widget_ops = array(
            'description' => esc_html__( 'Filter Jobs Widget.', 'iwjob'),
            'customize_selective_refresh' => true,
        );

        parent::__construct( 'iwj_job_filter', esc_html__( '[IWJ] Filter Jobs', 'iwjob'), $widget_ops );
    }
        ...
        foreach ($job_taxs as $job_tax_item) {
			...
			<p>
				...
				<select class="widefat" id="<?php echo $field_id_hide_empty; ?>" name="<?php echo $this->get_field_name( $key_hide_empty_item ); ?>">
					<option <?php echo ($value == '0') ? 'selected' : ''; ?> value="0"><?php echo __('No', 'iwjob'); ?></option>
					<option <?php echo ($value == '1') ? 'selected' : ''; ?> value="1"><?php echo __('Yes', 'iwjob'); ?></option>
				</select>
				<!--<input type="text" id="<?php /*echo $field_id_hide_empty; */?>" name="<?php /*echo $this->get_field_name( $key_hide_empty_item ); */?>" value="<?php /*echo $value; */?>" style="width:100%" />-->
			</p>
            <hr>
            <?php
        }
