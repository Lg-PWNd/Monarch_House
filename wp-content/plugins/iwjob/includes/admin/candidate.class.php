...
class IWJ_Admin_Candidate{
    ...
    static function register_metabox(){
        ...
        add_meta_box('iwj-employer-box', __('Candidate Metabox Info', 'iwjob'), array( __CLASS__, 'metabox_html'), 'iwj_candidate', 'normal', 'high');
    }
        ...
        <div class="iwj-metabox wp-clearfix">
            <table class="form-table">
                ...
                foreach (self::$fields as $field){
                    if(!in_array($field['id'], array('user_id','website', 'email', IWJ_PREFIX.'birthday',IWJ_PREFIX . 'gender',IWJ_PREFIX . 'languages', IWJ_PREFIX.'headline', IWJ_PREFIX.'address', IWJ_PREFIX.'public_account'))){
                        continue;
                    }
            ...
            <h3><?php echo __('Portfolio Gallery', 'iwjob'); ?></h3>
            <table class="form-table">
                <?php
                foreach (self::$fields as $field){
            ...
            <?php do_action('iwj_admin_candidate_form_after_gallery', $post_id); ?>

            <h3><?php echo __('Video URL', 'iwjob'); ?></h3>
            <table class="form-table">
                <?php
                foreach (self::$fields as $field){
            ...
            <h3><?php echo __('Social Infomation', 'iwjob'); ?></h3>
            <table class="form-table">
                <?php
                foreach (self::$fields as $field){
        ...
        if(isset($_POST) && $_POST && !defined( 'DOING_AJAX' ) && isset($_POST[IWJ_PREFIX.'headline']) && is_blog_admin()){
            if(get_post_type($post_id) == 'iwj_candidate'){
                ...
                if($thumbnail_id){
                    ...
                    $avatar = get_user_meta($user->ID, IWJ_PREFIX.'avatar', true);
                    if($avatar && is_numeric($avatar)){
                        delete_user_meta($user->ID, IWJ_PREFIX.'avatar');
                    }
                }
