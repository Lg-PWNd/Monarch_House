...
class IWJ_Order{
    ...
    public function can_pay($key){
        if($this->get_author_id() != get_current_user_id()){
            return new WP_Error(1, __('You have not permission to pay this order.', 'iwjob'));
        }
        ...
        if(($this->get_type() == 2 || $this->get_type() == 3) && !$this->get_job()){
            return new WP_Error(3, __('Sorry can not pay this order because miss job related.', 'iwjob'));
        }
    ...
    static function get_status_array(){
        return array(
            ...
            'iwj-cancelled' => __('Cancelled', 'iwjob'),
