...
class IWJ_Controller {
	...
	static function findjob_map(){
		...
		if($array_data && count($array_data) != '0'){
			...
			if ( isset($_GET['iwj_locations']) && $_GET['iwj_locations'] ){
				$term = get_term( $_GET['iwj_locations'] );
	...
	static function check_permission() {
		...
		if ( $post && $post->ID == iwj_get_page_id( 'dashboard' ) ) {
			...
			if(iwj_woocommerce_checkout()){
				if(isset($_GET['iwj_tab']) && isset($_GET['job-id'])){
					if($_GET['iwj_tab'] == 'renew-job'){
						$job_id = $_GET['job-id'];
						$job = IWJ_Job::get_job($job_id);
						if($job && $job->get_author_id() == get_current_user_id() && iwj_option('renew_job_price') > 0){
							$user_package = $job->get_user_package();
							if(!$user_package || $user_package->get_remain_renew_job() <= 0){
								$iwj_woocommerce = new IWJ_Woocommerce();
								$iwj_woocommerce->add_to_cart('renewjob', $_GET['job-id']);
							}
						}
					}elseif($_GET['iwj_tab'] == 'make-featured'){
						$job_id = $_GET['job-id'];
						$job = IWJ_Job::get_job($job_id);
						if($job && $job->get_author_id() == get_current_user_id()){
							$user_package = $job->get_user_package();
							if(!$user_package || !$user_package->can_make_featured() && iwj_option('featured_job_price') > 0){
								$iwj_woocommerce = new IWJ_Woocommerce();
								$iwj_woocommerce->add_to_cart('featuredjob', $_GET['job-id']);
							}
						}
					}
				}
	...
	static function publish_job() {
		//is admin or free job or single price with woocommerce mode
		if ( isset( $_GET['iwj_publish_job'] ) && $_GET['iwj_publish_job'] && (( current_user_can( 'administrator' ) || ! iwj_option( 'submit_job_mode' ) ) || (iwj_woocommerce_checkout() && iwj_option('submit_job_mode') == '2') )) {
			$job_id = $_GET['iwj_publish_job'];
			$job    = IWJ_Job::get_job( $job_id );

			if ( $job && $job->get_author_id() == get_current_user_id() && $job->has_status( 'draft' ) ) {
				//single price with woocommerce
				if(!current_user_can( 'administrator' ) && iwj_woocommerce_checkout() && iwj_option('submit_job_mode') == '2'){
					$iwj_woocommerce = new IWJ_Woocommerce();
					$iwj_woocommerce->add_to_cart('newjob', $job_id);
				}else{
					//is admin or free job mode
					if(! iwj_option( 'submit_job_mode' ) && !current_user_can( 'administrator' )){
						update_post_meta( $job_id, IWJ_PREFIX . 'free_job', '1' );
					}

					if ( current_user_can( 'administrator' ) || iwj_option( 'new_free_job_auto_approved' ) ) {
	...
	static function form_action() {
		if ( isset( $_POST ) && $_POST ) {
			if ( isset( $_POST['iwj-submit-job'] ) && wp_verify_nonce( $_POST['iwj-submit-job'], 'iwj-select-package' ) && iwj_option('submit_job_mode') == '1' ) {
				...
				if ( isset( $_POST['package'] ) && $_POST['package'] ) {
					...
					if ( $package->can_buy() ) {
						if(!$package->is_free() && iwj_option('woocommerce_checkout')){
							...
							$order = IWJ_Order::get_order( $order_id );
							//if order price <=0 then purchased order now

							$dashboard_url = iwj_get_page_permalink( 'dashboard' );

							//if free package then purchased package now
							if ( $package->is_free() ) {
								//purchased package
								$user_package = IWJ_User_Package::get_user_package( $user_package_id );
								$user_package->purchased();

								//if auto approved job => publish free job
								$job = IWJ_Job::get_job( $job_id );
					...
					if ( $user_package->can_submit() ) {
						...
						$remain_job = get_post_meta( $_POST['user_package'], IWJ_PREFIX . 'remain_job', true );
				...
				if ( $job ) {
					$user_package = $job->get_user_package();
					if ( $user_package && $user_package->get_remain_job() > 0 ) {
						update_post_meta( $user_package->get_id(), IWJ_PREFIX . 'remain_job', $user_package->get_remain_job() - 1 );
				...
				if ( isset( $_POST['package'] ) && $_POST['package'] ) {
					...
					$package    = IWJ_Package::get_package( $package_id );
					if ( $package->can_buy() ) {
						if(!$package->is_free() && iwj_woocommerce_checkout() && $package->get_price() > 0){
							$iwj_woocommerce = new IWJ_Woocommerce();
	...
	static function login() {
		...
		if ( is_wp_error( $user_signon ) ) {
			echo json_encode( array(
				'loggedin' => false,
				'message'  => iwj_get_alert( __( 'Wrong username or password.', 'iwjob' ), 'danger' )
			) );
		} else {
			...
			$redirect_url = isset( $_POST['redirect_to'] ) ?  $_POST['redirect_to'] : '';
                        $redirect_url = apply_filters('iwj_redirect_to', $redirect_url, $user_signon);
	...
	static function register() {
		...
		if ( strlen( $user_name ) < 5 ) {
			$return['message'] = iwj_get_alert( __( 'Username must be at least 5 characters.', 'iwjob' ), 'danger' );
			$return['success'] = false;
			echo json_encode( $return );
			wp_die();
		}
		...
		if ( username_exists( $user_name ) ) {
			$return['message'] = iwj_get_alert( __( 'Username already exists.', 'iwjob' ), 'danger' );
			$return['success'] = false;
			echo json_encode( $return );
			wp_die();
		}
		...
		if ( $role == 'employer' ) {
			...
			if ( $id ) {
				$return['message'] = iwj_get_alert( __( 'Company name already exists.', 'iwjob' ), 'danger' );
				$return['success'] = false;
				echo json_encode( $return );
				wp_die();
			}
	...
	static function change_password() {
		...
		if ( IWJ_PREVIEW_MODE && ($user->user_login == 'employer' || $user->user_login == 'candidate') ) {
			$return['message'] = iwj_get_alert( __( 'Sorry you can not change password for this account.', 'iwjob' ), 'danger' );
			$return['success'] = false;
			echo json_encode( $return );
			exit;
		}
	...
	static function lostpass() {
		...
		if ( ! $user_login ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'Please enter Email Or Username.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
		...
		if ( is_email( $user_login ) ) {
			...
			if ( ! username_exists( $user_login ) ) {
				echo json_encode( array(
					'success' => false,
					'message' => iwj_get_alert( __( 'Invalid Username.', 'iwjob' ), 'danger' )
				) );
				exit;
			} else {
	...
	static function delete_account() {
		...
		$user = wp_get_current_user();
		if ( IWJ_PREVIEW_MODE && ($user->user_login == 'employer' || $user->user_login == 'candidate') ) {
			$return['message'] = iwj_get_alert( __( 'Sorry you can not delete this account.', 'iwjob' ), 'danger' );
			$return['success'] = false;
			echo json_encode( $return );
			exit;
		}
	...
	static function resetpass() {
		...
		if ( ! IWJ_User::can_reset_password( $user, $code ) ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'You have not permission to change password.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
	...
	static function update_profile() {
		...
		if ( current_user_can( 'create_iwj_jobs' ) ) {
			...
			if ( $employer->can_update() ) {
				...
				echo json_encode( array(
					'success' => false,
					'message' => __( 'You have not permission.', 'iwjob' )
				) );
				exit;
			}
	...
	static function submit_job() {
		...
		if ( $job ) {
			if ( $job->can_publish_draft() ) {
				...
				echo json_encode( array(
					'sucess'   => false,
					'message'  => iwj_get_alert( __( 'Sorry, You have not permission save job.', 'iwjob' ), 'danger' ),
					'redirect' => false,
					'id'       => $id,
				) );
				exit;
	...
	static function renew_job() {
		...
		if ( $job ) {
			if ( $job->can_renew() ) {
				...
				echo json_encode( array(
					'sucess'  => false,
					'message' => iwj_get_alert( __( 'You have not permission to do it.', 'iwjob' ), 'danger' ),
					'id'      => $id,
				) );
				exit;
			}
		...
		echo json_encode( array(
			'sucess'   => false,
			'message'  => iwj_get_alert( __( 'Job does not exist.', 'iwjob' ), 'danger' ),
			'redirect' => false,
			'id'       => $id,
		) );
		exit;
		...
		if ( $order ) {
			...
			if ( $can_pay === true ) {
				if ( $order->get_price() > 0 && ! $_POST['payment_method'] ) {
					echo json_encode( array(
						'success' => false,
						'message' => iwj_get_alert( __( 'Please select payment method.', 'iwjob' ), 'warning' )
					) );
					exit;
				} else {
					echo json_encode( array(
	...
	static function follow() {
		...
		if ( ! current_user_can( 'apply_job' ) ) {
			echo json_encode( array(
				'success' => false,
				'message' => __( 'Sorry, You have not permission to follow company.', 'iwjob' )
			) );
	...
	static function save_job() {
		...
		if ( ! current_user_can( 'apply_job' ) ) {
			echo json_encode( array(
				'success' => false,
				'message' => __( 'Sorry, You have not permission to save job.', 'iwjob' )
			) );
		...
		if ( $user->save_job( $id ) ) {
			echo json_encode( array(
				'success' => true,
				'message' => __( '<i class="ion-heart"></i> Saved job', 'iwjob' )
			) );
	...
	static function undo_save_job() {
		...
		if ( $user->undo_save_job( $id ) ) {
			echo json_encode( array(
				'success' => true,
				'message' => __( '<i class="ion-heart"></i> Save job', 'iwjob' )
			) );
	...
	static function view_resum() {
		...
		if ( ! iwj_option( 'view_free_resum' ) ) {
			if ( ! current_user_can( 'create_iwj_jobs' ) ) {
				echo json_encode( array(
					'success' => false,
					'message' => iwj_get_alert( __( 'Sorry, You have not permission to view resume.', 'iwjob' ), 'danger' )
				) );
	...
	static function save_resum() {
		...
		if ( ! iwj_option( 'view_free_resum' ) ) {
			if ( ! current_user_can( 'create_iwj_jobs' ) ) {
				echo json_encode( array(
					'success' => false,
					'message' => __( 'Sorry, You have not permission to save resume.', 'iwjob' )
				) );
		...
		if ( $user->save_resum( $resum_id ) ) {
			echo json_encode( array(
				'success' => true,
				'message' => __( '<i class="ion-android-send"></i> Saved resume', 'iwjob' )
			) );
	...
	static function undo_save_resum() {
		...
		if ( $user->undo_save_resum( $resum_id ) ) {
			echo json_encode( array(
				'success' => true,
				'message' => __( '<i class="ion-android-send"></i> Save resume', 'iwjob' )
			) );
	...
	static function edit_alert() {
		...
		if ( ! $alert->can_edit() ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'You have not permission to edit this alert.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
	...
	static function delete_alert() {
		...
		if ( ! $alert->can_delete() ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'You have not permission to delete this alert.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
	...
	static function delete_job() {
		...
		if ( ! $job->can_delete() ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'You have not permission to delete this job.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
	...
	static function update_application() {
		...
		if ( ! $application->can_update() ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'You have not permission to update this application.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
	...
	static function delete_application(){
		...
		if ( ! $applied->can_update() ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'You have not permission to delete this application.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
	...
	static function application_email() {
		...
		if ( ! $application->can_send_email_to_candidate() ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'You have not permission to send this messages.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
	...
	static function contact() {
		...
		if ( ! $subject ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'Please enter subject.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
		...
		if ( ! $message ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'Please enter message.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
	...
	static function review() {
		...
		if ( ! $title ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'Please enter title.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
		if ( ! $content ) {
	...
	static function edit_reply_review(){
		...
		if ( ! $reply_id ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'Reply is not exist.', 'iwjob' ), 'danger' )
			) );
	...
	static function delete_review(){
		...
		if ( ! $review->can_delete() ) {
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'You have not permission to delete this review.', 'iwjob' ), 'danger' )
			) );
			exit;
		}
	...
	function get_application_details() {
		...
		if ( $application ) {
			if ( $application->can_view() ) {
				...
				echo __( 'Sorry, you have not permission to view this application.', 'iwjob' );
			}
		} else {
	...
	function get_order_details() {
		...
		if ( $order ) {
			if ( $order->can_view() ) {
				...
				echo __( 'Sorry, you have not permission to view this order.', 'iwjob' );
			}
		} else {
	...
	static function indeed_load_data(){
		...
		if ( count( $get_content ) ) {
			...
			echo json_encode( array(
				'success' => false,
				'message' => iwj_get_alert( __( 'No job found.', 'iwjob' ), 'danger' )
			) );
	...
	static function jobs_export() {
		...
		if ( count( $job_type ) > 1 ) {
			...
			foreach ( $job_type as $type_job ) {
				...
				$corefield_head  = IWJ_Admin_Jobs_Tools::coreFields( $type_job );
				$extrafield_head = IWJ_Admin_Jobs_Tools::extraFields( $type_job );
				$termfield_head  = IWJ_Admin_Jobs_Tools::termFields( $type_job );
			...
			$corefield_head  = IWJ_Admin_Jobs_Tools::coreFields( $job_type[0] );
			$extrafield_head = IWJ_Admin_Jobs_Tools::extraFields( $job_type[0] );
			$termfield_head  = IWJ_Admin_Jobs_Tools::termFields( $job_type[0] );
			$header_row      = array_merge( $corefield_head, $extrafield_head, $termfield_head );
	...
	static function parse_data_to_import() {
		...
		$parserObj          = new SmackCSVParser();
		$get_screen_info    = IWJ_Admin_Jobs_Tools::getPostValues( sanitize_key( $eventKey ) );
