...
class IWJ_Resume_Package{
    ...
    static function get_package($post = '', $force = false){
        ...
        if($post_id){
            ...
            if($force || !isset(self::$cache[$post_id])){
                self::$cache[$post_id] = new IWJ_Resume_Package($post);
            }
    ...
    public function get_number_resume(){
        ...
        return get_post_meta($this->post->ID, IWJ_PREFIX.'number_resum', true);
    }

    public function is_featured(){

        return $this->get_id() == iwj_option('resum_package_featured_id') ? true : false;
    }

    public function is_free(){

        return $this->get_id() == iwj_option('free_resum_package_id') ? true : false;
    }

    public function can_buy(){
        if($this->is_free() && iwj_option('free_resum_package_times') > 0){
            global $wpdb;
            ...
            $sql = $wpdb->prepare($sql, 'iwj_u_package', 'publish', IWJ_PREFIX.'package_id', $this->get_id());
            $total = $wpdb->get_var($sql);
            if($total >= iwj_option('free_resum_package_times')){
                return false;
            }
        }
