	...
	function create_job_product($type = 'package', $package_id = null) {
	    switch ($type){
		    ...
		    case 'applyjob_package' :
			    $title = sprintf(__('Apply Job Package %s', 'iwjob'), get_the_title($package_id));
			    $price = get_post_meta($package_id, IWJ_PREFIX.'price', true);
			    break;
	        case 'newjob' :
        ...
        if($title){
            $new_post = array(
                'post_title' 		=> $title,
                'post_content' 		=> esc_html__('This is a variable product used for job processed with WooCommerce', 'iwjob'),
                'post_status' 		=> 'publish',
