...
if ( ! class_exists( 'IWJ_Jobs_Imports_Upload_Handler' ) ) {
	class IWJ_Jobs_Imports_Upload_Handler {
		...
		function __construct( $options = null, $initialize = true, $error_messages = null ) {
			...
			$this->options = array(
				...
				'inline_file_types'                => '/\.(csv|xml|zip|txt)$/i',
				// Defines which files (based on their names) are accepted for upload:
				'accept_file_types'                => '/\.(csv|xml|zip|txt)$/i',
				...
				//		'image_file_types' => '/\.(gif|jpe?g|png)$/i', // Image resolution restrictions:
				//		'max_width' => null, 'max_height' => null, 'min_width' => 1, 'min_height' => 1, // Set the following option to false to enable resumable uploads:
				...
				*/
				...
				'image_versions'                   => array(// The empty image version key defines options for the original image:
					'' => array(// Automatically rotate images based on EXIF meta data:
						'auto_orient' => true
					...
					*//*	'thumbnail' => array(// Uncomment the following to use a defined directory for the thumbnails
		...
		protected function get_upload_path( $file_name = null, $version = null ) {
			...
			$eventDir   = '';
			if ( $file_name ) {
				$eventkey        = hash_hmac( 'md5', "$file_name", 'secret' );
				$this->event_key = $eventkey;
				$eventDir        = $uploadpath . '/jobs_import_export/imports' . '/' . $eventkey;
				if ( ! is_dir( $eventDir ) ) {
					wp_mkdir_p( $eventDir );
				}
			}
		...
		protected function trim_file_name( $name, $type = null, $index = null, $content_range = null ) {
			...
			$file_name_arr = array();
			$file_name_csv = explode( ".csv", $name );
			$file_name_xml = explode( ".xml", $name );
			$file_name_txt = explode( ".txt", $name );
			$file_name_arr = explode( '.', $name );
			if ( in_array( 'csv', $file_name_arr ) ) {
				if ( ! is_array( $version_zero ) ) {
					return $file_name_csv[0] . '.csv';
				} // Code modified by Fredrick Marks
				else {
					return $file_name_csv[0] . '-1.csv';
				} // Code modified by Fredrick Marks
				//Edited original file ends
			}
			if ( in_array( 'xml', $file_name_arr ) ) {
				if ( ! is_array( $version_zero ) ) {
					return $file_name_xml[0] . ' .xml';
				} // Code modified by Fredrick Marks
				else {
					return $file_name_xml[0] . '-1.xml';
				} // Code modified by Fredrick Marks
				//Edited original file ends
			}
			if ( in_array( 'txt', $file_name_arr ) ) {
				if ( ! is_array( $version_zero ) ) {
					return $file_name_txt[0] . 'txt';
				} else {
					return $file_name_txt[0] . '-1.txt';
				}
			}
			if ( $file_name_arr[1] == 'zip' ) {
				if ( ! is_array( $version_zero ) ) {

					return $file_name_arr[0] . ' .zip';
				} else {
					return $file_name_arr[0] . '-1.zip';
				}
			}
		...
		protected function is_valid_image_file( $file_path ) {
			if ( ! empty( $this->options['image_file_types'] ) ) {
		...
		protected function handle_file_upload( $uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null ) {
			...
			$file->type     = $type;
			$file->eventkey = $this->event_key;
			$file_extension = pathinfo( $file->name, PATHINFO_EXTENSION );
			if ( $file_extension == 'xml' ) {
				libxml_use_internal_errors( true );
				libxml_clear_errors();
				$xmlobj = simplexml_load_file( $uploaded_file );
		...
		public function checkFileExists( $fileName ) {
			...
			$file_with_path = IWJ_IMPORT_DIR . '/' . $fileName;
		...
		public function post( $print_response = true ) {
			if ( isset( $_REQUEST['_method'] ) && sanitize_text_field( $_REQUEST['_method'] ) === 'DELETE' ) {
	...
	new IWJ_Jobs_Imports_Upload_Handler();
}
