...
class IWJ_User{
    ...
    static function get_avatar($avatar = '', $id_or_email, $size = 96, $default = '', $alt = ''){
        ...
        $avatar = "<img alt='" . esc_attr( $alt ) . "' src='" . esc_url( $avatar_url ) . "' srcset='".$avatar_url2x." 2x' class='avatar avatar-{$size}{$author_class} photo' height='{$size}' width='{$size}' />";
    ...
    function get_employer($force = false){
        $employer_id = $this->get_employer_id();
        if($employer_id){
            return IWJ_Employer::get_employer($employer_id, $force);
        }
    ...
    function count_jobs_with_status($status = ''){
	    ...
	    if ( $status == 'expired' ) {
		    ...
		    $sql = $wpdb->prepare( $sql, $this->get_id(), 'iwj_job', $status );
	    }
    ...
    public function can_view_resum($id){
        ...
        if(iwj_option('view_free_resum')
            || user_can($this->user,'privilege_view_resum')
            || ($post->post_author == $this->get_id())
            || (user_can($this->user,'create_iwj_jobs') && $this->is_viewed_resum($id))
        ){
            return true;
        }
