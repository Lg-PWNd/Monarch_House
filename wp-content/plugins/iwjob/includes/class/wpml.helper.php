    ...
    if($sitepress){
        ...
        if ( WPML_LANGUAGE_NEGOTIATION_TYPE_DIRECTORY === (int) $sitepress->get_setting( 'language_negotiation_type' )){
            ...
            if($code == $default_language){
                $home_replace = iwj_get_actual_wpml_url($home_url);
            }else{
                $home_replace = iwj_get_actual_wpml_url($home_url).'/'.$code;
            }
            return str_replace($home_url, $home_replace, $current);
...
function iwj_insert_term_translate($term_id, $lang_code, $key, $translate){
    global $wpdb;
    if($wpdb->insert(
        $wpdb->prefix.'iwj_term_translates',
        array(
            'term_id' => $term_id,
            'lang_code' => $lang_code,
            'translate_key' => $key,
...
function iwj_insert_post_translate($post_id, $lang_code, $key, $translate){
    global $wpdb;
    if($wpdb->insert(
        $wpdb->prefix.'iwj_post_translates',
        array(
            'post_id' => $post_id,
            'lang_code' => $lang_code,
            'translate_key' => $key,
                        ...
                        $html .= '<ul>';
                            foreach($languages as $language){
                                if(isset($_SERVER['QUERY_STRING']) && $_SERVER['QUERY_STRING']){
                                    $url = $language['url'] .'?' . $_SERVER['QUERY_STRING'];
                                }else{
                                    $url = $language['url'];
                                }
