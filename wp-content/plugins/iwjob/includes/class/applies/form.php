...
class IWJ_Apply_Form extends IWJ_Apply {
            ...
            <td colspan="1">
                <div class="iwjmb-input ui-sortable">
                    <select id="allow_candidate_apply_job" class="iwjmb-select" name="allow_candidate_apply_job">
                        ...
                        <option value="1" <?php selected($allow_candidate_apply_job, 1); ?>><?php echo __('Candidate has published profile [status: publish]', 'iwjob'); ?></option>
                        <option value="2" <?php selected($allow_candidate_apply_job, 2); ?>><?php echo __('Candidate has submited profile [status: pending, publish]', 'iwjob'); ?></option>
                    </select>
                </div>
            </td>
        </tr>
            ...
            if($field['type'] == 'file'){
                ...
                if(isset($field['pre_value']) && $field['pre_value']){
                    ...
                    foreach ($file_extensions as $file_extension){
                        ...
                        if(isset($all_mime_support[$file_extension])){
                            $mimes[$file_extension] = $all_mime_support[$file_extension];
                        }
                    }
    ...
    function submit_application(){
        ...
        if(!$job->can_apply()){
            echo json_encode(array(
                'success' => false,
                'message' => iwj_get_alert(__('This Job has been expired', 'iwjob'), 'danger')
            ));
        ...
        if($user && !$user->can_apply()){
            echo json_encode(array(
                'success' => false,
                'message' => iwj_get_alert(__('You have not permission apply this job', 'iwjob'), 'danger')
            ));
        ...
        if(iwj_option('apply_job_mode')){
	        ...
	        if(!$user_package_ids ){
		        echo json_encode(array(
			        'success' => false,
			        'message' => iwj_get_alert(__('You must buy Apply Job Package to apply job', 'iwjob'), 'danger')
		        ));
	        ...
	        if($validate === true){
		        if(count($can_view_packages) < 1){
			        echo json_encode(array(
				        'success' => false,
				        'message' => iwj_get_alert(__('Your Apply Job Package is pending or expired', 'iwjob'), 'danger')
			        ));
