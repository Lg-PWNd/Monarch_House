...
class IWJ_Email {
    ...
    static public function email_content(&$email, $type) {
        ...
        $email['subject'] = $smarty->fetch('string:'.$email['subject']);
        try {
            $emogrifier = new Emogrifier( $email_template, $css );
            $email['content'] = $emogrifier->emogrify();
            //$email['content'] = apply_filters('the_content', $email['content']);
        } catch ( Exception $e ) {
            echo $e->getMessage();exit;
            //$logger = iwj_get_logger();
    ...
    static public function get_email_admin_register($args) {
        ...
        if ($enable_email) {
            ...
            $args = array(
                ...
                'role' => in_array('iwj_employer', (array)$user->user->roles) ? _x('Employer', 'Role for email', 'iwjob') :  (in_array('iwj_candidate', (array)$user->user->roles) ? _x('Candidate', 'Role for email', 'iwjob') : implode(', ', $user->user->roles)),
