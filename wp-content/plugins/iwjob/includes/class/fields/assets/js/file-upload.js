...
jQuery( function ( $ ) {
	...
	UploadButton = views.UploadButton = Backbone.View.extend( {
		className: 'iwjmb-upload-area',
		tagName: 'div',
		template: wp.template( 'iwjmb-upload-area' ),
		render: function () {
		...
		initialize: function ( options ) {
			...
			this.el.id = _.uniqueId( 'iwjmb-upload-area-' );
			this.render();

			//Areas
			this.dropzone = this.el;
			this.browser = this.$( '.iwjmb-browse-button' )[0];
		...
		initUploader: function () {
			...
			if ( extensions ) {
				this.plupload.filters.mime_types = [{title: i18niwjmbMedia.select, extensions: extensions}];
			}
			...
			this.uploader.bind( 'FilesAdded', function ( up, files ) {
				_.each( files, function ( file ) {
					...
					attributes = _.extend( {
						...
						icon: i18niwjmbMedia.loadingUrl
		...
		getExtensions: function () {
			...
			_.each( mimeTypes, function ( current, index ) {
				if ( i18niwjmbMedia.extensions[current] ) {
					exts = exts.concat( i18niwjmbMedia.extensions[current] );
				}
			} );
			return exts;
		}
	 ...
	 */
	function init() {
		new FileUploadField( {input: this, el: $( this ).siblings( 'div.iwjmb-media-view' )} );
	}

	$( ':input.iwjmb-file_upload' ).each( init );
	$( '.iwjmb-input' )
		.on( 'clone', ':input.iwjmb-file_upload', init )
} );
