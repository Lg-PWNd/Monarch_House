...
class IWJ_User_Package{
    ...
    public function get_type_title(){
        ...
        if($type == 'job_package'){
	        ...
	        return __('Apply Job Package', 'iwjob');
        }
    ...
    public function can_submit(){
        if($this->get_remain_job() <=0 || $this->get_user_id() != get_current_user_id()){
            return false;
        }
    ...
    public function can_make_featured(){
        if($this->get_remain_featured_job() <=0 || $this->get_user_id() != get_current_user_id()){
            return false;
        }
    ...
    public function can_renew(){
        if($this->get_remain_renew_job() <=0 || $this->get_user_id() != get_current_user_id()){
            return false;
        }
    ...
    public function get_status($calculate = true){
        ...
        if($calculate && $status == 'publish'){
            if($type == 'job_package' && $this->get_remain_job() <= 0  && $this->get_remain_featured_job() <= 0 && $this->get_remain_renew_job() <= 0){
                return 'iwj-expired';
