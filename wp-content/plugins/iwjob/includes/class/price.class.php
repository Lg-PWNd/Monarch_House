...
class IWJ_Price
{
    ...
    public function __construct($amount, $currency = 'USD')
    {
        ...
        if (is_string($currency)) {
            $this->currency = (is_string($currency) ? new IWJ_Currency($currency) : $currency);
        } elseif ($currency instanceof IWJ_Currency) {
    ...
    public function format()
    {
        ...
        if(iwj_option('price_trim_zeros')){
            $format = preg_replace( '/' . preg_quote( $this->currency->getDecimalSeparator(), '/' ) . '0+/', '', $format );
        }
    ...
    public static function parse($str, $currency = 'USD')
    {
        ...
        return new IWJ_Price($str, $currency);
    }
}
