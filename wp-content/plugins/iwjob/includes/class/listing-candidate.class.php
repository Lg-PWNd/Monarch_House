...
class IWJ_Candidate_Listing {
    ...
    /**
     * Count candidate in taxonomy when ajax load
     *
     * @return array
     */
    public static function count_candidates_in_taxonomy( $data_filters ) {
    ...
    public static function get_query_candidates( $data_filters ) {
        $args = array(
        ...
        $args['posts_per_page'] = iwj_option('candidates_per_page', get_option('posts_per_page', 20));
        if(isset($data_filters['posts_per_page']) && $data_filters['posts_per_page']){
            $args['posts_per_page'] = $data_filters['posts_per_page'];
        }

        $args = apply_filters('iwj_hook_ajax_args_query_listing_candidate', $args);

        $query = new WP_Query( $args );

        $query = apply_filters('iwj_hook_ajax_query_listing_candidate', $query);
    ...
    /**
     * Add url params when filter ajax load
     *
     * @return string
     */
    public static function get_request_url( $data_filters ) {
        ...
        $taxonomy_names = iwj_get_candidate_taxonomies();
        ...
        $add_query_arr = apply_filters('iwj_candidate_url_request_args', $add_query_arr, $data_filters);
    ...
    public static function get_feed_url($filters = null){
        ...
        $filters = array('feed'=>'candidate_feed') + $filters;
