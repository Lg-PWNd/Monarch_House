...
class IWJ_Job{
    ...
    public function renew_link(){
        ...
        $url = add_query_arg(array('iwj_tab' => 'renew-job', 'job-id' => $this->get_id()), $dashboard_url);
        return $url;
    }
    ...
    public function publish_draft_link(){
        ...
        if(current_user_can('administrator') || !iwj_option('submit_job_mode') || (iwj_option('submit_job_mode') == '2' && iwj_woocommerce_checkout())){
            $url = add_query_arg(array('iwj_publish_job' => $this->get_id()), home_url('/'));
        }else{
            $dashboard_url = iwj_get_page_permalink('dashboard');
            if(iwj_option('submit_job_mode') == '1'){
                $url = add_query_arg(array('iwj_tab' => 'new-job', 'step' => 'select-package', 'job-id' => $this->get_id()), $dashboard_url);
            }else{
                $url = add_query_arg(array('iwj_tab' => 'new-job', 'step' => 'payment', 'job-id' => $this->get_id()), $dashboard_url);
            }
        }
    ...
    public function make_featured_link(){
        $dashboard_url = iwj_get_page_permalink('dashboard');
        $url = add_query_arg(array('iwj_tab' => 'make-featured', 'job-id' => $this->get_id()), $dashboard_url);
        return $url;
    }
    ...
    public function set_featured($force = false){
    ...
    public function can_delete(){
        if(
            current_user_can('delete_iwj_job', $this->get_id()) && $this->has_status(array('publish', 'pending', 'expired', 'reject'))
        ){
            return true;
        }
    ...
    public function update($data){
        ...
        $user_package = $this->get_user_package();
        $max_categories = $user_package ? $user_package->get_max_categories() : '';
